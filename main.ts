...
class UniversityManagementSystem {
	...
	setGrade(studentId: number, courseId: number, grade: Grade): void {
		...
		this.grades.push({
			...
			grade,
			date: new Date(),
			semester: this.getCourseById(courseId)?.semester || Semester.First,
		});
	}
	...
	getTopStudentsByFaculty(faculty: Faculty): Student[] {
		...
		return this.students.filter(
			s => s.faculty === faculty && topStudentIds.has(s.id),
		);
	}
	...
	// Get course by ID
	private getCourseById(courseId: number): Course | undefined {
